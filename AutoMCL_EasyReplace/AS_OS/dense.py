...
def _declaration_dense(cfg, data, weight, bias=None, out_dtype=None):
    ...
    if sch is "4":
        #print("rpmmv")
        return _declaration_dense_right_pack(cfg, data, weight, bias, out_dtype)
    if sch is "6":
        #print("lpmm")
        return _declaration_dense_left_pack(cfg, data, weight, bias, out_dtype)
    if sch is "7":
        #print("tmm")
        return _declaration_dense_tmm(cfg, data, weight, bias, out_dtype)
    # if M <= 16:
    #     return _declaration_dense_nopack(cfg, data, weight, bias, out_dtype)
    return _declaration_dense_nopack(cfg, data, weight, bias, out_dtype)
...
@autotvm.register_topi_compute(nn.dense, "cpu", "direct_left_pack")
def _declaration_dense_left_pack(cfg, data, weight, bias=None, out_dtype=None):
    if out_dtype is None:
        out_dtype = data.dtype
    M, K = get_const_tuple(data.shape) # batch, in_dim
    N, _ = get_const_tuple(weight.shape) # out_dim
    # create tuning space
    cfg.define_split("tile_y", 32 if isinstance(M, tvm.expr.Var) else M, num_outputs=3, policy="factors")
    cfg.define_split("tile_x", 32 if isinstance(N, tvm.expr.Var) else N, num_outputs=3, policy="factors")
    cfg.define_split("tile_k", 32 if isinstance(K, tvm.expr.Var) else K, num_outputs=2, policy="factors")
    if cfg.is_fallback:
        _default_dense_pack_config(cfg, M, N, K)

    packD_bn = cfg["tile_y"].size[-1]
    packD_shape = (M // packD_bn, K, packD_bn)
    packD = tvm.compute(packD_shape,
                        lambda z, y, x: data[z * packD_bn + x, y], name="packed_data")

    k = tvm.reduce_axis((0, K), name="k")
    C = tvm.compute((M, N),
                    lambda y, x: tvm.sum(
                        packD[tvm.indexdiv(y, packD_bn), k, tvm.indexmod(y, packD_bn)].astype(dtype) * weight[x, k],
                        axis=k),tag="dense_left_pack")
    if bias is not None:
...
@autotvm.register_topi_compute(nn.dense, "cpu", "direct_right_pack")
def _declaration_dense_right_pack(cfg, data, weight, bias=None, out_dtype=None):
    if out_dtype is None:
        out_dtype = data.dtype
    M, K = get_const_tuple(data.shape) # batch, in_dim
    N, _ = get_const_tuple(weight.shape) # out_dim
...
def _declaration_dense_nopack(cfg, data, weight, bias=None, out_dtype=None):
    ...
    #print(cfg["tile_k"].size[0],cfg["tile_k"].size[1])
    #print("vec",vec)
...
@autotvm.register_topi_compute(nn.dense, "cpu", "direct_nopack_tmm")
def _declaration_dense_tmm(cfg, data, weight, bias=None, out_dtype=None):
    if out_dtype is None:
        out_dtype = data.dtype
    M, K = get_const_tuple(data.shape)
    N, _ = get_const_tuple(weight.shape)
    ...
    C = tvm.compute((M, N), lambda i, j: tvm.sum(data[i, z] * weight[j, z], axis=z), name='C',tag="dense_nopack_tmm")
    if bias is not None:
        C = tvm.compute((M, N), lambda i, j: C[i, j] + bias[j].astype(out_dtype),
                        tag=tag.BROADCAST)
...
def _schedule_dense(cfg, outs):
    ...
    def _callback(op):
        if "dense_left_pack" in op.tag:
            _schedule_dense_left_pack_template(cfg, s, op.output(0))
        elif 'dense_nopack' in op.tag:
            _schedule_dense_nopack_template(cfg, s, op.output(0))
        elif 'dense_right_pack' in op.tag:
            _schedule_dense_right_pack_template(cfg, s, op.output(0))
        elif 'dense_right_pack' in op.tag:
            _schedule_dense_right_pack_template(cfg, s, op.output(0))
        elif 'dense_nopack_tmm' in op.tag:
            _schedule_dense_nopack_tmm_template(cfg, s, op.output(0))
...
def _schedule_dense_nopack(cfg, outs):
    ...
    def _callback(op):
        if 'dense_nopack' in op.tag:
            _schedule_dense_nopack_template(cfg, s, op.output(0))
        if 'dense_nopack_tmm' in op.tag:
            _schedule_dense_nopack_tmm_template(cfg, s, op.output(0))
    traverse_inline(s, outs[0].op, _callback)
    return s


def _schedule_dense_right_pack_template(cfg, s, C):
    #print("_schedule_dense_left_pack_template")
...
def _schedule_dense_left_pack_template(cfg, s, C):
    #print("_schedule_dense_left_pack_template")
    packedD, weight = s[C].op.input_tensors
    CC = s.cache_write(C, "global")
    y, x = s[C].op.axis
    k, = s[CC].op.reduce_axis
    yt, yo, yi = cfg["tile_y"].apply(s, C, y)
    ...
    z, y, x = s[packedD].op.axis
    s[packedD].reorder(z, x, y)
    s[packedD].parallel(z)
    # 20201104 fixed add vec for y
    s[packedD].vectorize(y)
    return s
...
def _schedule_dense_nopack_tmm_template(cfg, s, C):
    #print("_schedule_dense_nopack_tmm_template")
