...
def _schedule_conv(s, cfg, data, data_pad, data_im2col, kernel_im2col, CC, Out, last):
    print("im2col dnmm schedule")
    # fetch schedule
...
def _schedule_conv_NCHWc(s, cfg, data, conv_out, last):
    # fetch schedule
    ow_bn, oc_bn, unroll_kw = cfg["tile_ow"].size[-1], cfg["tile_oc"].size[-1], cfg["unroll_kw"].val
    ...
    C, Out = conv_out, last
    CC = C.op.input_tensors
    data_im2col, kernel_im2col = s[CC].op.input_tensors
    x, z = s[data_im2col].op.axis
    # xo, xi = s[data_im2col].split(x, factor=ow_bn)
    # zo, zi = s[data_im2col].split(z, factor=ic_bn)
    xt, xo, xi = cfg["tile_ow"].apply(s, data_im2col, x)
