    ...
    def _intrin_func(ins, outs):
        def _instr(index):
            ...
            pair_reduction = tvm.call_llvm_intrin('int16x32',
                                                  'llvm.OptSchedule.avx512.pmaddubs.w.512',
                                                  tvm.const(0, 'uint32'),
                                                  vec_a, vec_b)
            quad_reduction = tvm.call_llvm_intrin('int32x16',
                                                  'llvm.OptSchedule.avx512.pmaddw.d.512',
                                                  tvm.const(0, 'uint32'),
...
def dot_16x1x16_uint8_int8_int32_cascadelake():
    ...
    def _intrin_func(ins, outs):
        def _instr(index):
            ...
            vnni_inst_name = 'llvm.OptSchedule.avx512.vpdpbusd.512'
            ...
            if llvm_id != 0: # VNNI is available for current LLVM version
                ...
                quad_reduction = tvm.call_llvm_intrin('int32x16',
                                                      'llvm.OptSchedule.avx512.vpdpbusd.512',
                                                      tvm.const(0, 'uint32'),
                                                      vec_zero,
                                                      vec_ai32, vec_bi32)
