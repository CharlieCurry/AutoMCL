...
def ttmm(M, K, N, dtype):
    data = tvm.placeholder((M, K), name='data', dtype=dtype)
    weight = tvm.placeholder((N, K), name='weight', dtype=dtype)
    weight_T = tvm.compute((K, N), lambda i, j: weight[j, i], name='weight_T')
    z = tvm.reduce_axis((0, K), name='z')
    C = tvm.compute((M, N), lambda i, j: tvm.sum(data[i, z] * weight_T[z, j], axis=z), name='C')
    s = tvm.create_schedule(C.op)
    z = s[C].op.reduce_axis[0]
    x, y = s[C].op.axis
    cfg = autotvm.get_config()
    cfg.define_split("tile_x", x, num_outputs=3, policy="factors")
    cfg.define_split("tile_y", y, num_outputs=3, policy="factors")
    cfg.define_split("tile_z", z, num_outputs=2, policy="factors")
    ...
    s[C].parallel(xyt)
    # s[C].parallel(xyo)
    # data, weight = s[C].op.input_tensors
...
if __name__ == '__main__':
    ...
    K = int(sys.argv[2])
    N = int(sys.argv[3])
    mytuner(n_trial, M, K, N, ttmm, 400)
'''
